...
class FieldReference(models.Model):
    ...
    This can be used by Staff, Users, Courses, or any other model in the system.
    """

    # Which app/model does this belong to?
    content_type = models.ForeignKey(ContentType, on_delete=models.CASCADE)
    object_id = models.PositiveIntegerField(null=True, blank=True)
    content_object = GenericForeignKey('content_type', 'object_id')
    
    # Which field in that model?
    ...
    class Meta:
        ...
        indexes = [
            ...
            models.Index(fields=['content_type', 'field_name', 'is_active']),
        ]
        verbose_name = "Field Reference"
        verbose_name_plural = "Field References"
    
    def __str__(self):
        model_name = self.content_type.model_class().__name__ if self.content_type else 'Unknown'
        return f"{model_name}.{self.field_name}: {self.value}"
    
    @classmethod
    def get_options(cls, model, field_name, search='', use_cache=True):

        # Get options for a specific model and field with optional search and caching
        content_type = ContentType.objects.get_for_model(model)
        cache_key = f"field_options_{content_type.id}_{field_name}_{search}"
